...
DEMYX_MARIADB_VERSION=$(docker exec -t "$DEMYX_REPOSITORY" mysql --version | awk -F '[ ]' '{print $6}' | awk -F '[,]' '{print $1}' | sed 's/-MariaDB//g' | sed -e 's/\r//g')
...
git add VERSION
git commit -m "ALPINE $DEMYX_ALPINE_VERSION, MARIADB $DEMYX_MARIADB_VERSION"
# Commit the rest
git add .
